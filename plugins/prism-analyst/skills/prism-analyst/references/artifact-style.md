...
- Deal name in large white sans-serif
...
Select by the metric actually available - skip a chart type if there's no data for it rather than rendering it empty:

| Metric domain | Chart | Notes |
|---|---|---|
| NAV / returns | Line chart | Annotate total return alongside the trend |
| Sector / geography allocation | Horizontal stacked bar or pie | Matches the Composition agent's output |
| Top holdings | Horizontal bar, ranked by value | |
| Leverage / coverage | Grouped bar | Actual vs. covenant limit |
| Cash flow | Waterfall | Opening NAV -> gains/losses -> distributions -> closing NAV |
...
- Delta indicators: green arrow for positive >15%, red arrow for negative >15% (in addition to the `>>>`/`<<<`/`~`/`!` text flags used in plain-chat responses)
