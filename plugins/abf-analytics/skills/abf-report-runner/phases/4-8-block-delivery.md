...
4. **Visual** (when visuals are on). Produce the block's default visual per `../../abf-gateway/knowledge/reference/visuals-playbook.md` for the chart *type*, and apply `../../abf-gateway/knowledge/reference/artifact-style.md` for its colours, typography, and layout — **mandatory: load it before rendering, use its palette and CSS variables, never free-style. Modality-independent, thinking mode on or off** (same discipline as the source link below).
