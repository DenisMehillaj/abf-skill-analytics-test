...
3. Deliver the answer. It is INCOMPLETE until it ends with one source link per retrieval it drew on: `Source — [<name>](<url>)` — a markdown link, analytics display name as visible text, the response's top-level `url` field (a sibling of `csv`, not inside the CSV text) as the href, copied exactly. Mandatory in the same turn, **thinking mode on or off** — never stop before it, never wait to be asked. See `knowledge/contracts/response-discipline.md` §Analytical answer shape.
