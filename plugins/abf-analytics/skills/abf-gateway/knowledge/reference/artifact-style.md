...
**MANDATORY house style — not a default to opt into.** Every rendered artifact this plugin produces — an inline HTML/React dashboard, an exported Word or PDF report, an Excel export, or a standalone chart image — MUST apply the palette, typography, layout, and CSS variables below. Load this file **before** rendering and use its tokens; never free-style colours or fall back to a generic palette. Like the source-link duty, this is **modality-independent** and holds with thinking/reasoning mode on OR off — shipping an artifact with off-brand styling is the same class of defect as shipping one with no source link. Plain chat-markdown replies are out of scope **for styling**; those follow `../contracts/response-discipline.md`.
